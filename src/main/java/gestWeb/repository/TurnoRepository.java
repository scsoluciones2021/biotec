...
public interface TurnoRepository extends JpaRepository<Turno, Long> {
    ...
    @Query(value = "SELECT * " +
     ...
     "and p.estado in (2, 3, 4) " +
     " order by dia, hora ", 
      nativeQuery = true)
	Page<Turno> busqueda_medico(LocalDate fecha, Long profesional, List<String> esp, Integer estado,
			Pageable pageable);

    @Query(value = "SELECT EXISTS(SELECT idconsulta FROM {h-schema}tur_prof_pac_cons WHERE idturno = ?1)", nativeQuery = true)
    Integer tieneConsulta(Long id);
