...
//, nativeQuery = true es cuando escribo la consulta directamente como sql 
    ...
    @Query(value = "SELECT concat(SUBSTRING_INDEX(nombre_actual,?2,-1),',', nombre_original) FROM adjuntos_ficha p WHERE p.id_ficha = ?1", nativeQuery = true)
    String[] obtenerArchivosFichaConsulta(Long idFicha, String separador);
}
