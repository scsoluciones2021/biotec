...
public class BloqueosResource {
    ...
    @GetMapping("/bloqueosDesde")
    @Timed
    public ResponseEntity<List<BloqueosDTO>> obtenerBloqueosDesde(@RequestParam String fechaDesde) {
        log.debug("REST request to get a list of Bloqueos from date");

      List<BloqueosDTO> bloqueosDesde = this.bloqueosService.obtenerBloqueosDesde(fechaDesde);

      return new ResponseEntity<List<BloqueosDTO>>(bloqueosDesde,  HttpStatus.OK);
